...
export function ReportPreviewModal({ isOpen, onClose, data }) {
  const [reportTitle, setReportTitle] = useState("불량 내역서");
        ...
        <div id="print-area" className="p-8 overflow-y-auto bg-white flex-1 custom-scrollbar print:p-0 print:overflow-visible">
          <div id="report-content-body" className="bg-white p-4">
            <div className="border-b-2 border-stone-800 pb-6 mb-6 flex justify-between items-end">
              <div><h1 contentEditable suppressContentEditableWarning className="text-3xl font-black text-stone-900 tracking-tight mb-2 outline-none p-1 -ml-1 rounded hover:bg-stone-100/50 focus:bg-stone-100 transition-colors" onBlur={(e) => setReportTitle(e.target.innerText)}>{reportTitle}</h1><p className="text-sm text-stone-500 font-medium ml-1">작성일: {new Date().toLocaleDateString()}</p></div>
