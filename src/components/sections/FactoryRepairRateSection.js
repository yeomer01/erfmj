              ...
              <div className="flex justify-between items-start mb-3">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <h4 className="font-bold text-stone-900 text-sm truncate max-w-[140px]" title={stat.vendor}>{stat.vendor}</h4>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                ...
                <div className={`flex items-center justify-center w-10 h-10 rounded-full font-black text-xs ${stat.rate === 100 ? 'bg-emerald-100 text-emerald-600' : stat.rate >= 50 ? 'bg-blue-100 text-blue-600' : 'bg-amber-100 text-amber-600'}`}>
                  {stat.rate}%
                </div>
              </div>
