                ...
                filteredItems.map(item => (
                  <tr key={item.id} className="hover:bg-purple-50/20 transition-colors group">
                    ...
                    <td className="px-5 py-3 text-center">
                      {hasWriteAccess ? (
                        <input type="text" key={`${item.id}-${item.manager}`} defaultValue={item.manager || ''} onBlur={(e) => handleManagerUpdate(item, e.target.value)} placeholder="-" className="w-full text-center bg-transparent border-b border-transparent hover:border-stone-300 focus:border-purple-500 focus:outline-none text-xs p-1 transition-colors" />
                      ) : (
                    ...
                    <td className="px-5 py-3 text-center flex gap-2 justify-center">
                      {hasWriteAccess && (
                        <>
                          <button onClick={() => onProcess(item)} className="px-3 py-1.5 bg-purple-600 text-white hover:bg-purple-700 rounded-md text-[11px] font-bold transition-all shadow-sm">차감 확정</button>
                          <button onClick={() => onCancelRequest(item)} className="px-3 py-1.5 bg-white text-stone-500 hover:text-stone-800 border border-stone-200 hover:border-stone-300 rounded-md text-[11px] font-bold transition-all">취소</button>
                        </>
                      )}
                    </td>
                  </tr>
