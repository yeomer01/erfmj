...
export const SplitProcessedSection = forwardRef(({ repairItems, deductionItems, deductionRepairItems, repaymentItems, defaultOpen, selectedRowIds, toggleRowSelection, toggleAllRows, onRevert, onRepayment, onRepairAfterDeduction, hasWriteAccess, vendorConfig }, ref) => {
  ...
  const totalDeductionAmount = useMemo(() => {
    return deductionItems.reduce((sum, item) => {
            ...
            items.map(item => (
              <tr key={item.id} className={`hover:bg-stone-50 transition-colors group ${item.isRepaid ? 'bg-green-50/30' : ''}`}>
                ...
                <td className="px-1.5 py-1.5 text-center overflow-hidden">
                  {hasWriteAccess ? (
                    <input type="text" key={`${item.id}-${item.manager}`} defaultValue={item.manager || ''} onBlur={(e) => handleManagerUpdate(item, e.target.value)} placeholder="-" className="w-full text-center bg-transparent border-none focus:ring-0 text-xs p-0" />
                  ) : (
